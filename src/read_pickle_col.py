...
def check_columns_in_database(
    pickle_file_path, database_name, table_name, ignore_column=None
):
    """
    ...
    if no_data_columns:
        print(
            "Columns that contain no data (all values are blanks or nulls), excluding the ignored column:"
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print(
            "Usage: python script_name.py <pickle_file_path> <database_name> <table_name> [ignore_column]"
        )
        sys.exit(1)
    ...
    check_columns_in_database(
        pickle_file_path, database_name, table_name, ignore_column
    )
