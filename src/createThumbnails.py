...
def create_thumbnail_with_imagemagick(
    input_path, output_path, size="128x128", error_log_path="error_log.txt"
):
    """Creates a thumbnail of the image at input_path using ImageMagick."""
    try:
        subprocess.run(
            ["convert", input_path, "-resize", size, output_path],
            check=True,
            capture_output=True,
            text=True,
        )
...
def traverse_and_create_thumbnails_with_imagemagick(
    root_dir,
    extensions,
    thumbnail_dir,
    max_thumbnails=None,
        ...
        for file in files:
            ...
            if file.split(".")[-1].lower() in extensions:
                ...
                output_path = os.path.join(
                    thumbnail_dir + "/" + first_char, os.path.basename(file)
                )
                os.makedirs(os.path.dirname(output_path), exist_ok=True)
                create_thumbnail_with_imagemagick(
                    input_path, output_path, error_log_path=error_log_path
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Create thumbnails for image files using ImageMagick."
    )
    parser.add_argument(
        "input_directory", help="The root directory to search for image files."
    )
    parser.add_argument(
        "output_directory", help="The directory where thumbnails will be saved."
    )
    parser.add_argument(
        "-n",
        "--number",
        type=int,
        default=None,
        help="Maximum number of thumbnails to create. Defaults to creating thumbnails for all images.",
    )
    parser.add_argument(
        "--error-log", default="error_log.txt", help="Path to the error log file."
    )
    ...
    traverse_and_create_thumbnails_with_imagemagick(
