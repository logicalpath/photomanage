...
def traverse_and_create_thumbnails(
        ...
        for file in files:
            ...
            if file_extension in extensions:
                ...
                output_path = os.path.join(
                    thumbnail_dir + "/" + first_char, "thumb-" + os.path.basename(file)
                )
...
def main():
    ...
    image_extensions = list(
        set(["heic", "jpg", "nef", "png", "aae", "jpeg", "tif", "tiff"])
    )

    # Call the function to start creating thumbnails using ImageMagick
    traverse_and_create_thumbnails(
        args.input_directory, image_extensions, args.output_directory, args.number
    )
