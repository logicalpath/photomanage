...
def analyze_outputs():
    ...
    if prompt_tps_readings:
        gpu_stats["prompt_tps_avg"] = sum(prompt_tps_readings) / len(
            prompt_tps_readings
        )
    ...
    if generation_tps_readings:
        gpu_stats["generation_tps_avg"] = sum(generation_tps_readings) / len(
            generation_tps_readings
        )
    ...
    if prompt_token_counts:
        gpu_stats["prompt_tokens_avg"] = sum(prompt_token_counts) / len(
            prompt_token_counts
        )
    if generation_token_counts:
        gpu_stats["generation_tokens_avg"] = sum(generation_token_counts) / len(
            generation_token_counts
        )
...
def analyze_logs(all_logs=False):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if batch_match:
                    ...
                    if detail:
                        errors.append(
                            f"Batch {batch_num} (exit {error_code}): {detail}"
                        )
                    else:
    ...
    log_label = (
        f"{len(logs_to_analyze)} log files" if all_logs else logs_to_analyze[0].name
    )
    result = {
...
def main():
    parser = argparse.ArgumentParser(
        description="Progress summary for image description generation"
    )
    parser.add_argument(
        "--all",
        action="store_true",
    ...
    if output_stats:
        ...
        if (
            output_stats["avg_time"] is not None
            and output_stats["median_time"] is not None
            and output_stats["min_time"] is not None
            and output_stats["max_time"] is not None
        ):
            print("\nGeneration time per image:")
            print(f"  Average:            {output_stats['avg_time']:.2f}s")
            print(f"  Median:             {output_stats['median_time']:.2f}s")
            print(
                f"  Range:              {output_stats['min_time']:.2f}s - {output_stats['max_time']:.2f}s"
            )
        else:
            print("\nGeneration time per image:")
        ...
        if output_stats.get("gpu"):
            ...
            if "generation_tps_avg" in gpu:
                print(
                    f"  Generation:         {gpu['generation_tps_avg']:.1f} tokens/sec avg "
                    f"({gpu['generation_tps_min']:.1f} - {gpu['generation_tps_max']:.1f})"
                )
            if "prompt_tps_avg" in gpu:
                print(
                    f"  Prompt processing:  {gpu['prompt_tps_avg']:.1f} tokens/sec avg "
            ...
            if "prompt_tokens_avg" in gpu or "generation_tokens_avg" in gpu:
                ...
                print(
                    f"  Tokens per image:   {prompt_avg:.0f} prompt + {gen_avg:.0f} generation"
                )
    ...
    if log_stats:
        ...
        if "gpu_tps" in log_stats:
            ...
            print(
                f"  Range:              {gpu['min']:.1f} - {gpu['max']:.1f} tokens/sec"
            )
